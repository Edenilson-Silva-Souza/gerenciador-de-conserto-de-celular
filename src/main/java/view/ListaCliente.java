...
public class ListaCliente {
	...
	public ListaCliente() {
		...
		janela.setTitle("Lista Clientes");
	...
	public void tabela(JFrame janela) {
		...
		funcionario = new JTextField("ID Cliente", 20);
		...
		bntRemover.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (funcionario.getText().equals("ID Cliente") || funcionario.getText().equals("")) {
					JOptionPane.showMessageDialog(janela, "Preencha o campo com ID válido");
				} else {
					Central central = new Central();
					central.removerCliente(Integer.parseInt(funcionario.getText()));
	...
	public void placehoder(JTextField funcionario, JPanel painelRemoverF) {
		funcionario.addFocusListener(new FocusAdapter() {
			...
			public void focusGained(FocusEvent e) {
				if (funcionario.getText().equals("ID Cliente")) {
					funcionario.setText("");
				}
			}
			...
			public void focusLost(FocusEvent e) {
				if (funcionario.getText().isEmpty()) {
					funcionario.setText("ID Cliente");
				}
			}
		});
	...
	public void atualizar(JTable tabela, JFrame janela) {
		...
		List<Cliente> usuarios = central.listaCliente();
		central.getSession().close();

		for (Cliente user : usuarios) {

			lista.add(new Object[] { user.getId() , user.getNome(), user.getCpf()});
		}
